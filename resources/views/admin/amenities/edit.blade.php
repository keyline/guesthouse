...
@section('page-title', 'Edit '.$amenity->name)
...
@section('header-actions')
    ...
    <button type="submit" form="amenity-form" class="inline-flex h-10 items-center rounded-lg bg-sky-600 px-4 text-sm font-bold text-white shadow-sm transition hover:bg-sky-700">Save Amenity</button>
@endsection
...
@section('content')
    ...
    <form id="amenity-form" method="POST" action="{{ route('admin.amenities.update', $amenity) }}">
        @csrf
        @method('PUT')
        ...
        <div class="mt-6 flex justify-end gap-3">
            ...
            <button class="h-11 rounded-lg bg-sky-600 px-5 text-sm font-bold text-white transition hover:bg-sky-700">Save Amenity</button>
        </div>
    </form>
@endsection
